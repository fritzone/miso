...
Achieving the simplicity of Qt's signal syntax seemed to not to be possible without using an extra step in the compilation phase (I am thinking at `moc`) and personally I have found including a function signature in the declaration of my signal not be working for me, so I went for the simplest syntax that was able to express the desired type of my signal (such as a signal, having a `float` and an `int` parameter) and with the supporting help of the variadic templates introduced in C++11 this seemed to be the ideal combination.
...
The parameters to the internal function come up as, using forward on the `f` parameter to the current function, then `slot_holders` which is a local variable of type:
...
```
Reading this method, it is obvious that the main action happens here, i.e. the actual call of a slot as per the corresponding signal takes place in these lines. 

A bit more investigation of this structure gives us the declaration of `FT` being a function which at compile time identifies its return type from the template parameter of the `slot_holder` class (`T` which is supposed to be a "Callable") which is fed into the `std::result_of` of the `<type_traits>` header having the parameters `Args...` of the signal class, this `slot_holder` resides in, combined again with the `Args...` of the signal to obtain a fully understandable expression.
...
```
which roughly holds the lowest level of a slot, i.e.: a function object, whether it is active or not and its address.
...
- The type of the `static SHT sh;` local variable came in via the template parameters, and for our case it will be structure `slot_holder` declared in the `signal` class. Now, this `sh` (slot holder, for the uninitiated) variable will be common for all the `connect_i` functions sharing a common prototype (hence, the static)
...
If you wonder about the `<<` operator in the macro definition of emit, please note the signal class has a very complex friend, in the form of:
